...
static int dsi_panel_power_off(struct dsi_panel *panel)
{
	...
	if (panel->delayms_after_resetlow) {
		DSI_DEBUG("delayms_after_resetlow = %u\n", panel->delayms_after_resetlow);
...
int dsi_panel_set_backlight(struct dsi_panel *panel, u32 bl_lvl)
{
	...
	if(panel->lhbm_config.enable) {
		if (bl_lvl) {
			...
			DSI_INFO("bl_lvl 0, keep last dbv_level:%d\n", panel->lhbm_config.dbv_level);
		}
	}
...
static int dsi_panel_parse_reset_sequence(struct dsi_panel *panel)
{
	...
	rc = utils->read_u32(utils->data, "qcom,delayms_after_resetlow",
					&panel->delayms_after_resetlow);
	if (!rc){
		DSI_DEBUG("[%s] delayms_after_resetlow = %u\n", panel->name, panel->delayms_after_resetlow);
	}
