...
void dsi_panel_aod_backlight_update(struct dsi_panel *panel, enum dsi_cmd_set_type type){
	...
	if (panel->bl_config.aod_brightness_updated > panel->aod_config.hig_nit)
		update_backlight_reg = panel->aod_config.hig_bl_reg;
	else if (panel->bl_config.aod_brightness_updated > panel->aod_config.min_nit)
		update_backlight_reg = panel->aod_config.mid_bl_reg;
	else
		update_backlight_reg = panel->aod_config.min_bl_reg;
	DSI_INFO("aod_brightness = %d\n update_backlight_reg = %d\n",panel->bl_config.aod_brightness_updated, update_backlight_reg);

	for (i =0; i < count; i++) {
		payload = (u8 *)cmds->msg.tx_buf;
		if (payload[0] == 0x51 && type == DSI_CMD_SET_CMD_SWITCH_IN) {
